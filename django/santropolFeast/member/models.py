...
class Member(models.Model):
    ...
    @property
    def cell_phone(self):
        return self.member_contact.all().filter(type=CELL).first().value or ''

    @property
    def work_phone(self):
        return self.member_contact.all().filter(type=WORK).first().value or ''

    @property
    def email(self):
        return self.member_contact.all().filter(type=EMAIL).first().value or ''
